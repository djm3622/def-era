...
Steps:
  1. Verify RAW_ZARR_DIR exists. This script does not download data.
...
if [[ ! -d "$raw_dir" ]]; then
    echo "Raw zarr not found at $raw_dir."
    echo "Download it separately first, then rerun this script."
    exit 1
fi

echo "Using existing raw zarr at $raw_dir."
echo "Preprocessing $raw_dir -> $processed_dir."
PYTHONPATH="$PWD/paradis/data:${PYTHONPATH:-}" "${PYTHON:-python3}" \
